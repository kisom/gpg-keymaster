...
// Imported from my keybase client code.
...
// command line.
func (keyRing *KeyRing) Unlock(keyID string, passphrase []byte) (err error) {
	e, ok := keyRing.Entities[strings.ToLower(keyID)]
...
func (keyRing *KeyRing) Sign(passphrase, message []byte, keyID string) (sig []byte, err error) {
	err = keyRing.Unlock(keyID, passphrase)
	if err != nil {
		return
	}
	...
	hdr := map[string]string{
		"Version": fmt.Sprintf("PGPutils (OpenPGP version %s)", Version),
	}
...
func NewEntity(name, email, outFile string) (ne *openpgp.Entity, err error) {
	ne, err = openpgp.NewEntity(name, "", email, PGPConfig())
	if err != nil {
		return
	}
	...
	err = ne.SerializePrivate(keyOut, PGPConfig())
	if err != nil {
		ne = nil
		return
	}
